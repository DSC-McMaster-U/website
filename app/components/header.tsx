...
const NavLink = ({ name, links }: NavLinkProps) => {
    return (
                    ...
                    <ul className="flex flex-col gap-y-4">
                        {links.map((link, index) => (
                            <li key={index} className="flex items-center">
                                <Link href={link.href} className="block whitespace-nowrap w-full dark:text-google-grey dark:hover:text-white transition-colors duration-200">
                                    <div className="flex flex-row items-center gap-x-2">
										{link.icon}
                                        <div className="flex flex-col">
                                            <span className="text-sm dark:text-white">{link.label}</span>
        ...
        <header className="flex flex-row justify-between fixed z-50 bg-google-black w-full p-4">
            <div className="flex flex-row items-center h-full gap-x-8">
                <Image
                    ...
                    height={50}
                />
                <nav className="lg:flex hidden flex-row">
                    <ul className="flex flex-row">
                        {navLinks.map((navLink, index) => (
                            <NavLink key={index} name={navLink.name} links={navLink.links} />
                        ))}
                    </ul>
                </nav>
            </div>
